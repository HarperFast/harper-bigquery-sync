...
const FLAGS = [
  'US', 'PA', 'LR', 'MH', 'BS', 'CY', 'MT', 'GR', 'SG', 'HK',
  'CN', 'JP', 'KR', 'GB', 'NO', 'DK', 'NL', 'DE', 'IT', 'FR'
];

const VESSEL_NAME_PREFIXES = [
  'PACIFIC', 'OCEAN', 'SEA', 'ATLANTIC', 'MARINE', 'GLOBAL',
  'STAR', 'CROWN', 'ROYAL', 'GOLDEN', 'SILVER', 'DIAMOND'
];

const VESSEL_NAME_SUFFIXES = [
  'TRADER', 'VOYAGER', 'SPIRIT', 'PIONEER', 'EXPLORER', 'NAVIGATOR',
  'GUARDIAN', 'PRINCE', 'QUEEN', 'KING', 'FORTUNE', 'GLORY'
];
...
export class VesselMetadataGenerator {
  ...
  generateVesselMetadata(mmsi) {
    ...
    const lastUpdated = new Date(
      this.startTime.getTime() + Math.random() * this.durationMs
    ).toISOString();
  ...
  generateDimensions(vesselType) {
    ...
    const length = Math.floor(
      ranges.length[0] + Math.random() * (ranges.length[1] - ranges.length[0])
    );

    const beam = Math.floor(
      ranges.beam[0] + Math.random() * (ranges.beam[1] - ranges.beam[0])
    );

    const draft = Math.floor(
      ranges.draft[0] + Math.random() * (ranges.draft[1] - ranges.draft[0])
    );
  ...
  generateOwner() {
    ...
    const ownerNames = [
      'Pacific', 'Atlantic', 'Global', 'International', 'United',
    ...
    for (const mmsi of this.mmsiList) {
      ...
      for (let i = 1; i < updatesPerVessel; i++) {
        const updateTime = new Date(
          this.startTime.getTime() + (this.durationMs / updatesPerVessel) * i
        ).toISOString();
