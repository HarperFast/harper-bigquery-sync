...
const MAJOR_PORTS = [
  ...
  { name: 'Tianjin', lat: 39.0842, lon: 117.2010, weight: 4, region: 'Asia' },
  ...
  { name: 'Los Angeles', lat: 33.7405, lon: -118.2720, weight: 6, region: 'Americas' },
  { name: 'Long Beach', lat: 33.7683, lon: -118.1956, weight: 5, region: 'Americas' },
  { name: 'New York/New Jersey', lat: 40.6700, lon: -74.0400, weight: 5, region: 'Americas' },
...
const FLAG_STATES = [
  'PA', 'LR', 'MH', 'HK', 'SG', 'MT', 'BS', 'CY', 'IM', 'GR',
  'CN', 'KR', 'JP', 'US', 'GB', 'NO', 'DE', 'IT', 'NL', 'DK'
];
...
class MaritimeVesselGenerator {
  ...
  initializeVesselPool() {
    ...
    for (let i = 0; i < cacheSize; i++) {
      ...
      const vessel = {
        ...
        length: Math.floor(Math.random() * (vesselType.lengthRange[1] - vesselType.lengthRange[0]) + vesselType.lengthRange[0]),
        beam: 0, // will calculate from length
        draft: parseFloat((Math.random() * (vesselType.draftRange[1] - vesselType.draftRange[0]) + vesselType.draftRange[0]).toFixed(1)),
        maxSpeed: vesselType.speedRange[1],
        cruiseSpeed: Math.floor(Math.random() * (vesselType.speedRange[1] - vesselType.speedRange[0]) + vesselType.speedRange[0])
      };
  ...
  generateMMSI() {
    ...
    const mids = [
      '201', '211', '219', '227', '232', '236', '240', '244', '247', // European countries
      '303', '310', '338', // North American countries
      '412', '413', '414', '416', '419', // Asian countries
      '563', '564', '565', '566', '567' // Southeast Asian countries
    ];
  ...
  generateVesselPosition(vessel, timestamp) {
    ...
    if (inPort || journey.inPort) {
      ...
      course = Math.floor(this.calculateBearing(
        journey.currentLat,
        journey.currentLon,
        journey.destination.lat,
        journey.destination.lon
