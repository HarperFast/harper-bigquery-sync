...
class MaritimeDataSynthesizer extends EventEmitter {
  ...
  async loadHistoricalData(days) {
    try {
      ...
      const recordsPerDay = Math.floor((24 * 60 * 60 * 1000) / this.config.generationIntervalMs) * this.config.batchSize;
  ...
  async start(options = {}) {
    ...
    try {
      ...
      console.log(`  Records per day: ~${Math.floor((24 * 60 * 60 * 1000) / this.config.generationIntervalMs) * this.config.batchSize}`);
