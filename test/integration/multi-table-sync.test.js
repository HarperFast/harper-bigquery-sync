...
describe('Multi-Table Sync Integration Tests', () => {
  describe('Configuration Loading', () => {
    it('should load multi-table configuration', async () => {
      ...
      const config = MULTI_TABLE_CONFIG;
    ...
    it('should wrap legacy single-table config in tables array', async () => {
      ...
      assert.strictEqual(normalizedConfig.bigquery.tables[0].targetTable, 'VesselPositions', 'Should default to VesselPositions table');
    });
...
async function syncTable(tableConfig, client) {
  const engineConfig = {
    bigquery: {
      projectId: 'test-project',
      credentials: '/path/to/key.json',
